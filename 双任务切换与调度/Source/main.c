...
{       
    ...
    if (currentTask == idleTask) 
    {
        ...
        if (taskTable[0]->delayTicks == 0)      //对所有任务进行扫描，看有没有延时PP
        {
    ...
    else 
    {
        ...
        if (currentTask == taskTable[0])    //PP这里分为三种情况，任务二；空闲任务；当前任务。
        {
...
** Returned value       :   无
***********************************************************************************************************/
void tTaskSystemTickHandler () //PP 延时切换中断中，所有的延时减1
{
    // 检查所有任务的delayTicks数，如果不0的话，减1。
    int i;
    for (i = 0; i < 2; i++)     //对所有任务进行扫描，看有没有延时PP
    {
